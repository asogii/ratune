...
fn handle_home_click(x: u16, y: u16, app: &mut App, center: ratatui::layout::Rect) {
    use crate::ui::home::compute_home_layout;
...
fn home_click_panel(x: u16, y: u16, area: Rect, panel: HomePanel, app: &mut App) {
    match panel {
        HomePanel::RecentAlbums => {
            let inner = crate::ui::home::home_panel_inner(area);
            let album_count = app.home.recent_albums.len();
            let Some(album_index) = crate::ui::home::home_recent_album_index_at(
                x,
                y,
                inner,
...
/// Thin wrapper — delegates to the per-tab keymap module.
fn map_key(
    code: KeyCode,
    modifiers: KeyModifiers,
    active_tab: Tab,
    ...
    keymap::map_key(code, modifiers, active_tab, kb, pending_gg)
}
