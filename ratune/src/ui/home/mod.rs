...
pub fn compute_home_layout(area: Rect, cfg: &Config) -> Option<HomeLayout> {
    ...
    if bottom_h == 0 {
        return Some(HomeLayout {
            top: top_area,
            bottom_left: Rect { x: area.x, y: area.y, width: 0, height: 0 },
            bottom_right: Rect { x: area.x, y: area.y, width: 0, height: 0 },
            top_panel,
            bottom_left_panel,
            bottom_right_panel,
            bottom_h: 0,
...
pub fn home_recent_album_index_at(
    x: u16, y: u16, inner: Rect, show_art: bool, scroll_offset: usize, album_count: usize,
) -> Option<usize> {
    if album_count == 0 || y < inner.y || y >= inner.y + inner.height || x < inner.x || x >= inner.x + inner.width {
        return None;
    }
    if !show_art {
...
pub fn render_home_tab(f: &mut Frame, area: Rect, app: &mut App, accent: Color, help_visible: bool) {
    let cfg = &app.config;
    let theme = app.theme.clone();
    let Some(layout) = compute_home_layout(area, cfg) else { return };
    let use_strip_graphics = app.home_strip_graphics_wanted(help_visible);
    render_home_panel(f, layout.top, layout.top_panel, app, accent, use_strip_graphics, &theme);
    if layout.bottom_h == 0 { return; }
    render_home_panel(f, layout.bottom_left, layout.bottom_left_panel, app, accent, use_strip_graphics, &theme);
    render_home_panel(f, layout.bottom_right, layout.bottom_right_panel, app, accent, use_strip_graphics, &theme);
}

fn render_home_panel(
    f: &mut Frame, area: Rect, panel: HomePanel, app: &mut App, accent: Color,
    use_strip_graphics: bool, theme: &Theme,
) {
    ...
    match panel {
        HomePanel::RecentAlbums => {
            ...
            if !app.config.home_recent_albums_show_art {
                render_recent_albums_list(f, albums_inner, &app.home.recent_albums,
                    app.home.album_scroll_offset, app.home.album_selected_index, accent, is_active);
                return;
            }
            if use_strip_graphics {
                ...
                render_art_strip_text_fallback(f, albums_inner, &app.home.recent_albums,
                    app.home.album_selected_index, accent, is_active);
            }
        }
        HomePanel::RecentTracks => render_recent_tracks_block(f, area, &app.home, accent, theme),
        HomePanel::Rediscover => render_rediscover_block(f, area, &app.home, accent, theme),
    }
}

fn render_recent_albums_list(
    f: &mut Frame, area: Rect, albums: &[RecentAlbum], scroll_offset: usize,
    selected_index: usize, accent: Color, is_active: bool,
) {
    if area.height == 0 || area.width == 0 { return; }
    if albums.is_empty() {
        f.render_widget(Paragraph::new(Line::from(Span::styled("  No album history yet", Style::default().fg(Color::DarkGray)))), Rect { height: 1, ..area });
        return;
    }
    ...
    for row in 0..max_items {
        let idx = scroll_offset + row;
        let a = &albums[idx];
        let text = format!(" {:>2}. {} {}", idx + 1, pad_or_truncate(&a.artist_name, artist_w), pad_or_truncate(&a.album_name, album_w));
        let selected = is_active && idx == selected_index;
        let style = if selected { Style::default().bg(accent).fg(Color::Black) } else { Style::default() };
...
fn render_art_strip_ratatui(f: &mut Frame, albums_inner: Rect, app: &mut App, _is_active: bool) {
    ...
    let Some(picker) = app.art_picker.as_ref() else { return };
    if albums_inner.height < 3 { return; }
    ...
    for i in 0..visible_count {
        ...
        if album_index >= app.home.recent_albums.len() { break; }
        ...
        if let Some(bytes) = app.home_art_cache.get(&album_id) {
            ...
            if app.home_strip_last_cells.get(&album_id).copied() != Some(slot) { app.home_strip_art.remove(&album_id); }
            ...
            if !app.home_strip_art.contains_key(&album_id) {
                if is_sixel && sixel_builds_this_frame >= MAX_SIXEL_STRIP_BUILDS_PER_FRAME { continue; }
                let prepared = crate::ui::art_prepare::prepare_art_image_for_rect_contain_fit(img, art_rect, fs);
                ...
                if is_sixel { sixel_builds_this_frame += 1; }
            }
            if let Some(state) = app.home_strip_art.get_mut(&album_id) {
                f.render_stateful_widget(StatefulImage::default().resize(img_resize.clone()), art_rect, state);
            }
        }
    }
...
fn render_art_strip_labels(f: &mut Frame, inner: Rect, home: &HomeState, accent: Color, _cell_px: Option<(u16, u16)>, is_active: bool) {
    ...
    for row_in_grid in 0u16..layout.grid_rows {
        ...
        for col_in_grid in 0..layout.per_row {
            ...
            let (name_style, artist_style) = if is_selected {
                (Style::default().fg(accent).add_modifier(Modifier::BOLD), Style::default().fg(accent))
            } else {
                (Style::default().fg(Color::Gray), Style::default().fg(Color::DarkGray))
            };
            let col_x = inner.x.saturating_add(layout.pad_x).saturating_add((col_in_grid as u16) * (thumb_cols + STRIP_GAP_COLS));
            f.render_widget(Paragraph::new(Line::from(Span::styled(name_label, name_style))), Rect { x: col_x, y: name_row_y, width: thumb_cols, height: 1 });
            if artist_row_y < inner.y + inner.height {
                f.render_widget(Paragraph::new(Line::from(Span::styled(artist_label, artist_style))), Rect { x: col_x, y: artist_row_y, width: thumb_cols, height: 1 });
            }
        }
    }
}

pub fn render_art_strip_text_fallback(f: &mut Frame, area: Rect, albums: &[RecentAlbum], selected_index: usize, accent: Color, is_active: bool) {
    ...
    for (i, album) in albums.iter().enumerate().take(visible) {
        ...
        spans.push(Span::styled(label, if selected { Style::default().bg(accent).fg(Color::Black) } else { Style::default().fg(Color::DarkGray) }));
    }
    f.render_widget(Paragraph::new(Line::from(spans)), Rect { height: 1, ..area });
    if area.height > 1 {
        if let Some(album) = albums.get(selected_index) {
            f.render_widget(Paragraph::new(Line::from(Span::raw(format!("  {} — {}", album.album_name, album.artist_name)))), Rect { y: area.y + 1, height: 1, ..area });
        }
    }
    if area.height > 2 && is_active {
        f.render_widget(Paragraph::new(Line::from(Span::styled("  h/l navigate  Enter play  a add to queue", Style::default().fg(Color::DarkGray)))), Rect { y: area.y + area.height.saturating_sub(1), height: 1, ..area });
    }
}
...
fn render_recent_tracks_block(f: &mut Frame, area: Rect, home: &HomeState, accent: Color, theme: &Theme) {
    ...
    if home.recent_tracks.is_empty() {
        lines.push(Line::from(Span::styled("  No play history yet", Style::default().fg(Color::DarkGray))));
    } else {
        for (i, record) in home.recent_tracks.iter().enumerate().take((inner.height as usize).min(home.recent_tracks.len())) {
            ...
            let text = format!(" {:>2}. {} {} {}", i + 1, pad_or_truncate(&record.track_name, track_w), pad_or_truncate(&record.artist_name, artist_w), rel);
...
fn render_rediscover_block(f: &mut Frame, area: Rect, home: &HomeState, accent: Color, theme: &Theme) {
    ...
    if home.rediscover.is_empty() {
        lines.push(Line::from(Span::styled("  Listen to more music to unlock suggestions", Style::default().fg(Color::DarkGray))));
    } else {
        for (i, (_, name)) in home.rediscover.iter().enumerate().take((inner.height as usize).saturating_sub(1).min(home.rediscover.len())) {
            lines.push(Line::from(Span::styled(format!(" {:>2}. {}", i + 1, truncate(name, inner.width as usize - 6)),
                if is_active && home.selected_index == i { Style::default().bg(accent).fg(Color::Black) } else { Style::default() })));
        }
    }
    if inner.height > 0 {
        while lines.len() < inner.height.saturating_sub(1) as usize { lines.push(Line::from("")); }
        lines.push(Line::from(Span::styled("  Press r to re-roll", Style::default().fg(Color::DarkGray))));
    }
    f.render_widget(Paragraph::new(lines), inner);
}
...
fn pad_or_truncate(s: &str, width: usize) -> String {
    ...
    else if w < width { let mut out = s.to_string(); for _ in 0..(width - w) { out.push(' '); } out }
    else { truncate(s, width) }
}
